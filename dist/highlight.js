...
__webpack_require__.d(__webpack_exports__, {
  "default": function() { return /* binding */ highlight; }
});
...
/* harmony default export */ var es_core = (core);

;// CONCATENATED MODULE: ./node_modules/highlight.js/es/languages/matlab.js
/*
Language: Matlab
...
/* harmony default export */ var languages_matlab = (matlab);

;// CONCATENATED MODULE: ./node_modules/highlight.js/es/languages/excel.js
/*
...
/* harmony default export */ var languages_excel = (excel);
...
/* harmony default export */ var languages_python = (python);
...
/* harmony default export */ var languages_php = (php);
 ...
 * @returns {string}
 */
function javascript_lookahead(re) {
  return javascript_concat('(?=', re, ')');
}
 ...
 * @returns {string}
 */
function javascript_concat(...args) {
  const joined = args.map((x) => javascript_source(x)).join("");
  return joined;
}
...
function javascript(hljs) {
    ...
    variants: [
      ...
      },
      {
        match: [
          /extends/,
          /\s+/,
          javascript_concat(IDENT_RE$1, "(", javascript_concat(/\./, IDENT_RE$1), ")*")
        ],
        scope: {
          1: "keyword",
  ...
  function noneOf(list) {
    return javascript_concat("(?!", list.join("|"), ")");
  }

  const FUNCTION_CALL = {
    match: javascript_concat(
      /\b/,
      noneOf([
        ...BUILT_IN_GLOBALS,
        "super"
      ]),
      IDENT_RE$1, javascript_lookahead(/\(/)),
    className: "title.function",
    relevance: 0
  };

  const PROPERTY_ACCESS = {
    begin: javascript_concat(/\./, javascript_lookahead(
      javascript_concat(IDENT_RE$1, /(?![0-9A-Za-z$_(])/)
    )),
    end: IDENT_RE$1,
    excludeBegin: true,
    keywords: "prototype",
  ...
  const FUNCTION_VARIABLE = {
    match: [
      ...
      IDENT_RE$1, /\s*/,
      /=\s*/,
      javascript_lookahead(FUNC_LEAD_IN_RE)
    ],
    className: {
      1: "keyword",
      3: "title.function"
  ...
  return {
    ...
    contains: [
      ...
      {
        className: 'attr',
        begin: IDENT_RE$1 + javascript_lookahead(':'),
        relevance: 0
      },
      FUNCTION_VARIABLE,
      { // "value" container
...
/* harmony default export */ var languages_javascript = (javascript);
...
function xml_stripOptionsFromArgs(args) {
  const opts = args[args.length - 1];
 ...
 * @returns {string}
 */
function xml_either(...args) {
  const opts = xml_stripOptionsFromArgs(args);
  const joined = '(' +
    (opts.capture ? "" : "?:") +
...
/* harmony default export */ var languages_xml = (xml);
 ...
 * @returns {string}
 */
function markdown_concat(...args) {
  const joined = args.map((x) => markdown_source(x)).join("");
  return joined;
}
...
function markdown(hljs) {
  ...
  const LINK = {
    variants: [
      ...
      {
        begin: markdown_concat(/\[.+?\]\(/, URL_SCHEME, /:\/\/.*?\)/),
        relevance: 2
      },
      // relative urls
      {
...
/* harmony default export */ var languages_markdown = (markdown);
 ...
 * @returns {string}
 */
function css_lookahead(re) {
  return css_concat('(?=', re, ')');
}
 ...
 * @returns {string}
 */
function css_concat(...args) {
  const joined = args.map((x) => css_source(x)).join("");
  return joined;
}
...
/* harmony default export */ var languages_css = (css);
...
es_core.registerLanguage('matlab', languages_matlab);
es_core.registerLanguage('excel', languages_excel);
es_core.registerLanguage('python', languages_python);
es_core.registerLanguage('php', languages_php);
es_core.registerLanguage('javascript', languages_javascript);
es_core.registerLanguage('xml', languages_xml);
es_core.registerLanguage('markdown ', languages_markdown);
es_core.registerLanguage('css', languages_css);
...
/* harmony default export */ var highlight = (es_core);
}();
